...
%   h_a_n_alt = "C:\Principal\IC\Estruturas_e_rotinas\TROTS\Dados\Head-and-Neck_Alt\Head-and-Neck-Alt_" + num2str(current_test) + ".mat";
    ...
    folder = [prostate liver h_a_n];
        for Path = folder
            for Difficulty = ["easy" "moderate" "hard"]
                if Path == prostate
                    Case = "Prostate";
                    if Difficulty == "easy"
                        dataID_TGoal = 1;
                ...
                elseif Path == h_a_n
                    ...
                    for Analysis = ["Absolute","Average"]
                        for Method = ["Interior point","Automatic"]
                            for p = 0.02
                                Counter = Counter +1;
                                Table = Solver(Difficulty, Table, Counter, Case, Path, Analysis, Method, p, dataID_TGoal, dataID_CUB, dataID_RUB);
                            end
                        end
                    end
            end
...
writetable(Table, 'Resultados.csv')
end