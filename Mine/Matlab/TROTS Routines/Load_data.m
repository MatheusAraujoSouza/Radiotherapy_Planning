...
% "criticals", and one "regular" structures. Those mat  rices are,
% respectively:
% 
% Tumor    -> AT_1 and AT_2
...
% each prescription vector and pencil-beam matrix are concatenaded:
%
...
for i = dataID_CUB 
    if i ~= 0
        [n_rows_A ,~] = size(data.matrix(i).A);
        ACritical = [ACritical; data.matrix(i).A];
        for j = 1:size_p
            if problem(j).dataID == i && problem(j).IsConstraint == 1      
...
for i = dataID_RUB
    if i ~= 0
        [n_rows_A ,~] = size(data.matrix(i).A);
        ARegular = [ARegular; data.matrix(i).A];
        for j = 1:size_p
            if problem(j).dataID == i && problem(j).IsConstraint == 1      
