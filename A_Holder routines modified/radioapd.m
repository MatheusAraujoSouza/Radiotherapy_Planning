...
T= sqrt(6*(mt+3));
su= ut-2*lt-lambda;
MM= sparse(A*A' + [speye(mt)/3 zeros(mt,mc+mg); zeros(mc+mg,mt) speye(mc+mg)]);
y= MM\([(sum(su)/(mt+3)-su)/6; zeros(mc,1); ones(mg,1)]);
v3= MM\[ones(mt,1)/T; ones(mc+mg,1)];
...
M=sparse(A'*sparse(diag(Dtil))*A+sparse(diag(D9)));
[L,q]=chol(M(p,p));
L=sparse(L);
dx=-(L\(L'\r(p)));
dx(p)= dx;
