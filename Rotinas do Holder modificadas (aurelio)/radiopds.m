...
tg = sparse(prescription(TumorRowIndex));
ut=sparse((1+p)*tg);
lt=sparse((1-p)*tg + tol);
uc= sparse(prescription(CriticalRowIndex));
ug= sparse(prescription(RegularRowIndex));
w= norm(lt, inf)/ tol;
...
pp= 100000;
we= sparse(w*ones(mt,1));
ec= sparse(ones(mc,1));
eg= sparse(ones(mg,1));

a=sparse(pp*ones(mt,1));
t=a;
c=sparse(pp*ones(mc,1));
g=sparse(pp*ones(mg,1));
x=sparse(pp*ones(n,1));
su=a;
sl=a;
sc=c;
sg=g;
